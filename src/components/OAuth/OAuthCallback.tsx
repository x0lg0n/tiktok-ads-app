    ...
    const handleCallback = async () => {
      ...
      if (error) {
        ...
        switch (error) {
          case "access_denied":
            userMessage =
              "You denied access to the application. Connect again and approve the requested permissions.";
            break;
          case "invalid_scope":
            userMessage =
          ...
          case "invalid_client":
            userMessage =
              "Invalid app configuration. Please check your TikTok app client ID and settings.";
            break;
          case "invalid_request":
            userMessage =
              "Invalid sign-in request. Please try again from the app.";
            break;
          case "invalid_grant":
            userMessage =
      ...
      if (!code || !state) {
        setStatus("error");
        setErrorMessage(
          "Invalid sign-in response. Please try connecting again."
        );
        setTimeout(() => navigate("/"), 3000);
        return;
      }
      ...
      try {
        ...
        const oauthSource = localStorage.getItem("oauthSource");

        if (pendingAd) {
          localStorage.removeItem("pendingAdData");
          setTimeout(() => navigate("/create-ad"), 1500);
        } else if (oauthSource === "connectButton") {
          localStorage.removeItem("oauthSource");
          setTimeout(() => navigate("/"), 1500);
        } else {
          // If user connected from login screen, go back to login screen
          const fromLogin = document.referrer.includes("/oauth/login");
          setTimeout(() => navigate(fromLogin ? "/oauth/login" : "/"), 1500);
        }
      } catch (err: unknown) {
    ...
    <div className="flex flex-col items-center justify-center min-h-screen p-5 text-center bg-gray-50">
      ...
      {status === "success" && (
        <div className="max-w-md">
          ...
          <h2 className="mb-2 text-2xl font-bold text-cyan-500">
      ...
      {status === "error" && (
        <div className="max-w-md">
          ...
          <h2 className="mb-2 text-2xl font-bold text-pink-500">
